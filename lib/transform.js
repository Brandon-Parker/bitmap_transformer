...
function extract(rawData) {
  ...
  for (var i = 0; i < extractedData.invertedPalette.length; i++) {
    var colorOffset = 54 + i * 4;
    var currentColor = extractedData.invertedPalette[i];
    rawData.writeUInt8(currentColor.blue, colorOffset);
    rawData.writeUInt8(currentColor.green, colorOffset + 1);
    rawData.writeUInt8(currentColor.red, colorOffset + 2);
  }
  debugger;
  return rawData;
}
