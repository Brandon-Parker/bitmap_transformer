...
const transformOption = process.argv[4];
...
function randomize(changedData, arrayOfColors, palette) {
  for (var i = 0; i < palette; i += 4) {
    arrayOfColors.push({
      blue: randomNumber0to255(),
      green: randomNumber0to255(),
      red: randomNumber0to255()
    });
  }
}
...
function extract(rawData) {
  ...
  if (transformOption === 'invert') {
    invert(rawData, extractedData.paletteColors, extractedData.paletteLength);
  } else if (transformOption === 'grayscale') {
    grayscale(rawData, extractedData.paletteColors, extractedData.paletteLength);
  } else {
    randomize(rawData, extractedData.paletteColors, extractedData.paletteLength);
  }
